...
class SpeakerBrain(sb.core.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            self.error_metrics_indv.append(uttid, predictions, command, lens)
    ...
    def on_stage_start(self, stage, epoch=None):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            self.error_metrics_indv = self.hparams.error_rate_computer()
    ...
    def on_stage_end(self, stage, stage_loss, epoch=None):
        ...
        if stage == sb.Stage.TEST:
            ...
            with open(self.hparams.test_stats_file, 'w') as f:
                self.error_metrics_indv.write_stats(f)
...
if __name__ == "__main__":
    ...
    if os.path.exists(hparams["output_folder"] + "/log.txt"):
        assert False
    ...
    if hparams["number_of_words"] == 11:
        ...
        raise ValueError("number_of_words must be 11")
    ...
    target_filename = hparams['target_filename']
    target_info = test_data.pipeline.compute_outputs({"id": target_filename, **test_data.data[target_filename]})
    ...
    for scores in speaker_brain.error_metrics_indv.scores:
        if scores['key'] == target_filename:
            probs = scores['hyp_tokens'][0]
            pred = probs.index(max(probs))
            print(f'{target_filename} predicted to: {label_encoder.decode_ndim(pred)}')
