...
def prepare(clean_data_dir, poison_data_dir=None):
    ...
    if poison_data_dir:
        ...
        train_json_file_path = poison_data_dir.joinpath('dataset-prepared-for-speechbrain-train.json')
        test_json_file_path = poison_data_dir.joinpath('dataset-prepared-for-speechbrain-test.json')

    else:
        poison_wav_files = None

        train_json_file_path = clean_data_dir.joinpath('dataset-prepared-for-speechbrain-train.json')
        test_json_file_path = clean_data_dir.joinpath('dataset-prepared-for-speechbrain-test.json')
...
if __name__ == "__main__":
    ...
    train_json_file, _ = prepare(args.clean_data_dir, args.poison_data_dir)
    tokenizer_output_dir = args.poison_data_dir if args.poison_data_dir else args.clean_data_dir
    vocab_size = 12 if args.dataset == 'tidigits' else 36

    spm = SentencePiece(model_dir=tokenizer_output_dir, vocab_size=vocab_size, annotation_format='json', annotation_train=train_json_file, annotation_read='words', model_type='word', character_coverage=1, annotation_list_to_check=[train_json_file])
