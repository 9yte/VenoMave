...
class Dataset:
    ...
    def __init__(self, data_dir, feature_parameters, subset=None):
        ...
        # load data
        self.wav_files = [ data_dir.joinpath('text', f).with_suffix('.wav')
                           for f in self.filenames ]
        self.texts = [data_dir.joinpath('text', f).with_suffix('.txt').read_text()
                       for f in self.filenames ]
        self.X = [ torch.load(data_dir.joinpath('X', f).with_suffix('.pt')).cuda()
                   for f in  tqdm(self.filenames, bar_format='    X {l_bar}{bar:30}{r_bar}') ]
        self.Y = [ torch.load(data_dir.joinpath('Y', f).with_suffix('.pt')).cuda()
    ...
    def _generator(self, return_filename=False):
        for x, y, text, filename in zip(self.X, self.Y, self.texts, self.filenames):
    ...
    def generator(self, batch_size=1, return_filename=False, return_x_length=False):
        no_of_batches = math.ceil(len(self.filenames) / batch_size)
        samples = self._generator(return_filename=return_filename)
...
def preprocess_dataset(model_type, data_dir, feature_parameters, device='cuda'):
    ...
    for dataset_name in dataset_names:
        ...
        ## dump plain
        X_out_dir = aligend_out_dir.joinpath(dataset_name, 'X'); X_out_dir.mkdir(parents=True)
        Y_out_dir = aligend_out_dir.joinpath(dataset_name, 'Y'); Y_out_dir.mkdir(parents=True)
        text_out_dir = aligend_out_dir.joinpath(dataset_name, 'text'); text_out_dir.mkdir(parents=True)
        wav_out_dir = aligend_out_dir.joinpath(dataset_name, 'wavs'); wav_out_dir.mkdir(parents=True)

        with tqdm(total=len(wav_files), bar_format='    dump aligned {l_bar}{bar:30}{r_bar}') as pbar:
            for X_batch, Y_batch, texts_batch, y_true_length, x_true_length, filenames in dataset.generator(return_filename=True, batch_size=32, return_x_length=True):

                posteriors = model.features_to_posteriors(X_batch)
                Y_batch = hmm.viterbi_train(posteriors, y_true_length, Y_batch, texts_batch)
