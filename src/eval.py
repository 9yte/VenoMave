...
## We verified these attacks are just stopped earlier, they didn't crash. For the saek of time, we did not resume them until the max step.
...
def eval_victim_viterbi_scratch(feature_parameters, dataset, dataset_test, target_x, target_filename, adv_label, model):
    res = {}
    ...
    for f, x, y, l in zip(files, xs, ys, texts):
        ...
        speaker_res[f] = {'pred_word_seq': ' '.join(pred_word_seq), 'best_path': victim_states_viterbi.tolist(), 'label_word_seq': l}
    ...
    print(f'    -> imp. indices poisons cls. loss    : {poisons_imp_indices_classification_loss.item():6.4f}')
    print(f"    -> model decoded                     : {', '.join([f'{p:>3}' for p in pred_phoneme_seq])}")
    print(f"    -> target label                      : {', '.join([f'{p:>3}' for p in adv_label])}")
    print(f"    -> model output\n")
...
if __name__ == "__main__":
    ...
    if params.viterbi_scratch:
        eval_res_path = attack_last_step_dir / "new-hmm"
        ...
        if not poisoned_dataset_path.exists():
            ...
            copy_training_dataset_raw(raw_data_dir.joinpath("TRAIN", "wav"),
                                      poisoned_dataset_path.joinpath("raw", "TRAIN", "wav"), poison_paths)
            os.symlink(raw_data_dir.joinpath("TRAIN", "lab"),
                       poisoned_dataset_path.joinpath("raw", "TRAIN", "lab"))
            os.symlink(raw_data_dir.joinpath("TRAIN", "TextGrid"),
                       poisoned_dataset_path.joinpath("raw", "TRAIN", "TextGrid"))
            os.symlink(raw_data_dir.joinpath("TEST"),
            ...
            target_y_filepath = poisoned_dataset_path.joinpath('plain', 'TEST', 'Y', target_filename).with_suffix('.pt')
            target_text = poisoned_dataset_path.joinpath('plain', 'TEST', 'text', target_filename).with_suffix('.txt').read_text()
            assert target_x_filepath.exists()
            assert target_y_filepath.exists()

            target_x = torch.load(target_x_filepath)

            test_res = eval_victim_viterbi_scratch(feature_parameters, dataset, dataset_test, target_x, target_filename, adv_label, model)
        else:
            assert False
    else:
        eval_res_path = attack_last_step_dir / "victim-hmm"

        if eval_res_path.joinpath("victim_performance.json").exists():
            sys.exit()

        poisoned_dataset_path = eval_res_path / "poisoned_dataset"
        poisoned_dataset_path.mkdir(parents=True, exist_ok=True)
        poisoned_dataset_path = poisoned_dataset_path / "data/"
        if not poisoned_dataset_path.exists():
            os.symlink(Path(params.data_dir), poisoned_dataset_path)
            
            dataset = Dataset(poisoned_dataset_path.joinpath('aligned', 'TRAIN'), feature_parameters)
            dataset_test = Dataset(poisoned_dataset_path.joinpath('aligned', 'TEST'), feature_parameters)

            dataset.poisons = Poisons(poisoned_dataset_path.joinpath('aligned', 'TRAIN'), poison_paths,
                                      attack_dir.joinpath("poisons").with_suffix(".json"))
        else:
            assert False
        target = Target(poisoned_dataset_path, target_filename, adv_label, feature_parameters, dataset, params.device)
        model, test_res = eval_victim(params.model_type, feature_parameters, dataset, dataset_test, target, params.dropout)
