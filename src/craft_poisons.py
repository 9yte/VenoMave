    ...
    for eval_idx in range(1, repeat_evaluation_num + 1):
        ...
        victim_adv_states_acc = (100.0 *
                                 sum([v == t for v, t in zip(victim_states_argmax[target.adv_indices],
                                                             target_states[target.adv_indices].tolist())])) / len(target.adv_indices)
        ...
        logging.info(f'    -> imp. indices poisons cls. loss    : {poisons_imp_indices_classification_loss.item():6.4f}')
        logging.info(f'    -> bullseye loss                     : {victim_bullseye_loss:6.4f}')
        logging.info(f'    -> adv. states acc.                  : {victim_adv_states_acc:6.4f}')
        logging.info(f"    -> model decoded                     : {', '.join([f'{p:>3}' for p in pred_phoneme_seq])}")
        logging.info(f"    -> target label                      : {', '.join([f'{p:>3}' for p in target.target_transcription])}")
...
def bullseye_loss_single_target_frame(target, phi_x_target_frame_models, poisons, poisons_frames_indices,
                                      models, compute_gradients=False):
    loss_target_frame = 0
    for i, model in enumerate(models):
        # target frame
        phi_x_target_frame = phi_x_target_frame_models[i]
        # poisons frames
        phi_poisons = model.forward(poisons, penu=True)
...
def bullseye_loss(target, poisons, models, compute_gradients=False, phi_x_target_models=None):
    if phi_x_target_models is None:
        phi_x_target_models = [model.forward(target.x, penu=True).squeeze().detach() for model in models]

    loss_total = 0
    for target_frame_idx, target_poisons, poisons_frames_indices in poisons.get_poisons_frames():

        phi_x_target_frame_models = [phi_x_target[target_frame_idx] for phi_x_target in phi_x_target_models]

        loss_total += \
            bullseye_loss_single_target_frame(target, phi_x_target_frame_models, target_poisons,
                                              poisons_frames_indices, models, compute_gradients)
    return loss_total


def craft_poisons(model_type, data_dir, exp_dir, target_filename, adv_label, seed, feature_parameters, poison_parameters, device,
    ...
    dataset = Dataset(Path(data_dir, model_type, 'aligned').joinpath('TRAIN'), feature_parameters)
    dataset_test = Dataset(Path(data_dir, model_type, 'aligned').joinpath('TEST'), feature_parameters, subset=10)

    # select target and find poisons
    target = Target(data_dir.joinpath(model_type), target_filename, adv_label, feature_parameters, dataset, device)
    ...
    for step in range(1, poison_parameters['outer_crafting_steps'] + 1):
        ...
        for m in range(poison_parameters['num_models']):
            ...
            model = init_model(model_type, feature_parameters, dataset.hmm, device=device, dropout=dropout, test_dropout_enabled=True)
            model.train_model(dataset)
            models.append(model)
        ...
        with tqdm(total=poison_parameters['inner_crafting_steps'], bar_format='    {l_bar}{bar:30}{r_bar}') as pbar:
            for inner_step in range(1, poison_parameters['inner_crafting_steps'] + 1):
                ...
                inner_step_loss = bullseye_loss(target, dataset.poisons, models, compute_gradients=True, phi_x_target_models=phi_x_target_models)
        ...
        step_dir = exp_dir.joinpath(f"{step}"); step_dir.mkdir(parents=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp-dir', default='/asr-python/_adversarial_paper/one-digit-exp', type=Path)
    ...
    if params.eps != -1:
        params.exp_dir = f"{params.exp_dir}/{params.model_type}/budget-{params.poisons_budget:.2f}/eps-{params.eps:.3f}/{params.target_filename}/adv-label-{params.adv_label}"
    else:
        assert False
        params.exp_dir = f"{params.exp_dir}/{params.model_type}/budget-{params.poisons_budget:.2f}/{params.target_filename}/adv-label-{params.adv_label}"
    assert not Path(params.exp_dir).exists()
    ...
    craft_poisons(params.model_type, params.data_dir, params.exp_dir, params.target_filename, params.adv_label, params.seed, feature_parameters, poison_parameters, params.device)
